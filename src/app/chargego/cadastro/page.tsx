...
export default function CadastroChargeGo() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch("http://localhost:8080/placasbaterias", cabecalho)

            if (response.ok) {
                alert(`ChargeGo cadastrada com sucesso`)
                setBateria({
                    id: 0,
                    tensao_nominal: 0,
                    capacidade_ah: 0,
            ...
            console.error("Erro ao cadastrar o ChargeGo:", error)
        }
    ...
    return (
        <main className="flex flex-col items-center justify-center">
            <h1 className="text-3xl text-center font-bold mb-2 text-indigo-600">Cadastro de ChargeGo</h1>
            <form className="w-1/3 m-auto border border-indigo-950 p-2 rounded-md" onSubmit={handleSubmit}>
                ...
                <button className="bg-green-700 text-white text-xl p-2 block ms-auto me-2 rounded-md" type="submit">Cadastrar ChargeGo</button>
            </form>
        </main>
    )
