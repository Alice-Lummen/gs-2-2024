...
export default function ChargeGo() {
    const [lista, setLista] = useState<BateriaProps[]>([]);

    const chamadaApi = async () => {
        const response = await fetch("http://localhost:8080/placasbaterias");
    ...
    return (
        <main className="flex flex-col items-center justify-center">
            <h1 className="text-3xl text-center font-bold mb-2 text-green-600">ChargeGo</h1>
                    ...
                    {lista.map((bateria) => (
                        <tr key={bateria.id} className="hover:bg-gray-100 transition duration-200">
                            ...
                            <td className="py-2 px-4 border-b text-left">
                                <Link href={`/chargego/${bateria.id}`} className="text-green-600 hover:text-green-800 font-semibold">
                                    Ver Detalhes
                                </Link>
                            </td>
                        </tr>
            ...
            <Link href="/chargego/cadastro" className="bg-green-800 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md mt-5">
                Cadastrar Novo ChargeGo
            </Link>
        </main>
    );
}