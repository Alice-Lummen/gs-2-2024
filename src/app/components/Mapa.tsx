...
export default function Mapa() {
  ...
  const mostrarDetalhes = (link: string) => {
      ...
      <div className="flex flex-col text-2xl font-bold h-64 md:h-[500px]">
        ...
        <ul className="flex flex-col gap-2 justify-evenly h-full">
          {pontos.map((ponto, index) => (
            <>
            <li key={index} onClick={() => mostrarDetalhes(ponto.link)} className="text-green-900 hover:text-green-700 cursor-pointer">{ponto.nome}</li>
